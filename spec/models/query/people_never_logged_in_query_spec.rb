  ...
  describe PeopleNeverLoggedInQuery do
    ...
    describe '#call' do
      it 'generates the expected sql' do
        expect(PeopleNeverLoggedInQuery.new.call.to_sql).to match_sql expected_sql
      end

      it 'returns an arel relation' do
        expect(PeopleNeverLoggedInQuery.new.call).to be_an_instance_of(Person::ActiveRecord_Relation)
      end
      ...
      it 'returns only those people who have never logged in' do
        ...
        expect(PeopleNeverLoggedInQuery.new.call).to match_array([p2, p3])
      end
