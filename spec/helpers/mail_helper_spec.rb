...
RSpec.describe MailHelper, type: :helper do
  ...
  describe '#easy_copy_link_to' do
    ...
    it 'returns link' do
      expect(subject).to have_link 'www.example.com', href: 'www.example.com'
    end

    it 'wraps link with padded div to ensure easy copy and paste in email clients (including lotus notes)' do
      expect(subject).to have_selector 'div'
      expect(subject).to have_selector('div[style*="padding: 10px 25px"]')
    end
  end
  ...
  describe '#app_guidance' do
    ...
    it 'returns expected text' do
      expect(subject).to include 'Find out more about how to use People Finder on the'
    end

    it 'includes link to intranet' do
      expect(subject).to have_link 'MoJ Intranet', href: 'https://intranet.justice.gov.uk/peoplefinder'
    end
  end
  ...
  describe '#do_not_reply' do
    ...
    it 'returns expected text' do
      expect(subject).to include 'This email is automatically generated. Do not reply'
    end
  end
