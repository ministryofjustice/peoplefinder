...
RSpec.describe AnalyticsHelper, type: :helper do
  ...
  describe '#edit_team_analytics_attributes' do
    it 'returns JSON including virtual pageview data expected by GA' do
      expect(edit_team_analytics_attributes(1)).to include "virtual-pageview": "/teams/edit-click"
    end

    it 'returns JSON event data for GA' do
      expect(edit_team_analytics_attributes(3)).to include "event-category": 'Edit team click', "event-action": "Click edit team 3"
    end
  end
end
