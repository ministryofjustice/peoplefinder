...
RSpec.describe SuggestionFormBuilder, type: :form_builder do
  ...
  describe "#check_box" do
    subject(:check_box) { builder.check_box(:test_field) }
    ...
    it "returns govuk styled check box" do
      expect(check_box).to eql output
    end
    ...
    it "adds a selectable block-label inside the form-group" do
      expect(check_box).to match(/.*form-group.*<label class="block-label selection-button-checkbox" for="suggestion_test_field">.*<\/label>/)
    end

    it "adds a checkbox inside the label" do
      expect(check_box).to match(/<label.*<input type="checkbox" value="1" name="suggestion\[test_field\]" id="suggestion_test_field" \/>.*<\/label>/)
    end

    it "adds label from translation defined in specific scope" do
      expect(check_box).to include "My test label"
    end
  end
end
