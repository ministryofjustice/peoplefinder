...
describe "Token Authentication" do
  ...
  describe "bad email from valid domain" do
    it_behaves_like "it received a valid request from" do
  ...
  it "logging in with a fake token" do
    ...
    expect(page).not_to have_text("Start building your profile now")
  ...
  context "when logging in with a valid token" do
    let(:ff31) { "Mozilla/5.0 (Windows NT 5.2; rv:31.0) Firefox/31.0" }
  ...
  it "requesting more than 8 tokens per hour isn't permitted" do
    1.upto(9) do |count|
      ...
      if count < 9
        ...
        expect(page).not_to have_text("reached the limit")
      else
