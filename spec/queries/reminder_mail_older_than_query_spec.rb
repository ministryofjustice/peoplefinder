  ...
  describe '#call' do
    it 'generates the expected sql' do
      Timecop.freeze(Time.utc(2017, 1, 19, 12, 26, 1)) do
        expect(ReminderMailOlderThanQuery.new(5.days.ago).call.to_sql).to match_sql expected_sql
      end
    end

    it 'returns an arel relation' do
      expect(ReminderMailOlderThanQuery.new(3.days.ago).call).to be_an_instance_of(Person::ActiveRecord_Relation)
    end
    ...
    it 'returns expected records' do
      ...
      expect(ReminderMailOlderThanQuery.new(6.days.ago).call).to match_array([p2])
    end
