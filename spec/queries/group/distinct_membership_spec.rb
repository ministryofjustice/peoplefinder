...
module Queries
  module Group
    describe DistinctMembership do
      ...
      it 'includes all the non leadership members and concatenates role names' do
        query = DistinctMembership.new(group: group, leadership: false)
        result = query.call
      ...
      it 'returns only those records with leadership status' do
        query = DistinctMembership.new(group: group, leadership: true)
        result = query.call
      ...
      it 'can be chained' do
        result = DistinctMembership.new(group: group, leadership: false).call.where(given_name: 'Stephen')
        expect(result.to_a.size).to eq 1
        expect(result.first.name).to eq 'Stephen Richards'
      end
    end
