...
describe PeopleLoggedInAtLeastOnceQuery do
  ...
  describe '#call' do
    it 'generates the expected sql' do
      expect(PeopleLoggedInAtLeastOnceQuery.new.call.to_sql).to match_sql expected_sql
    end

    it 'returns an arel relation' do
      expect(PeopleLoggedInAtLeastOnceQuery.new.call).to be_an_instance_of(Person::ActiveRecord_Relation)
    end
    ...
    it 'returns only those people who have logged in' do
      ...
      expect(PeopleLoggedInAtLeastOnceQuery.new.call).to match_array([p1, p4])
    end
