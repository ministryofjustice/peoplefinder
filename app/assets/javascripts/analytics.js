...
$(function(){
  ...
  ifPage('home-page', function (){
    ...
    sendEvent('form.search-box', 'submit', 'home-page', 'home-page-search', 'User made a search from the home page');
    sendEvent('.leaf-node a.subteam-link', 'click', 'home-page', 'home-page-used-the-org-browser', 'User went to a team page from the org browser');
    sendEvent('.add-new-person', 'click', 'home-page', 'home-page-add-new-profile', 'User clicked on add new profile');

    $('.new-org-browser').click(function (ev){
      var $self = $(ev.target).closest('.new-org-browser');
      if( !$self.hasClass('tracked-for-click') ){
        ga('send', 'event', 'home-page', 'home-page-explored-the-org-browser', 'User clicked on the org browser');
  ...
  ifPage('search-page', function (){
    ...
    var multiValue = {
      label: 'search-page-multiple-results', text: 'More than two results found for keyword'
    };

    var resultCount = $('.search-result').length;

    if( resultCount < 3 ){
      ga('send', 'event', 'search-page', values[resultCount].label, values[resultCount].text);
    } else {
      ga('send', 'event', 'search-page', multiValue.label, multiValue.text);
    }


    var clickValues = [
      { label: 'search-page-click-first-item',  text: 'User clicked on the first search result' },
      { label: 'search-page-click-second-item', text: 'User clicked on the second search result' },
      { label: 'search-page-click-third-item',  text: 'User clicked on the third search result' },
      { label: 'search-page-click-past-third-item',  text: 'User clicked on a search result past the third one' }
    ];
    ...
    $('.search-result a').click(function (ev){
      ...
      if( index < 3 ){
        ga('send', 'event', 'search-page', clickValues[index].label, clickValues[index].text);
      } else {
